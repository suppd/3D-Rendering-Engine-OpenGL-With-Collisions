...
:	_name( pName ), _transform( glm::translate( pPosition ) ), _parent(nullptr), _children(),
    _mesh( nullptr ),_behaviour( nullptr ), _target( nullptr ), _material(nullptr), _world(nullptr)
...
void GameObject::setLocalRotation(glm::vec3 pRotation)
{
    _transform[1] = glm::vec4(pRotation, 1);
}
...
void GameObject::setBehaviourTarget(GameObject* ptarget, AbstractBehaviour* pBehaviour) {
    _target = ptarget;
    pBehaviour->setTarget(ptarget);
}
...
glm::vec3 GameObject::getScale() const {
    glm::vec3 scalevalue;
    scalevalue.x = glm::length(glm::vec3(_transform[0]));
    scalevalue.y = glm::length(glm::vec3(_transform[1]));
    scalevalue.z = glm::length(glm::vec3(_transform[2]));
    return scalevalue;
}
