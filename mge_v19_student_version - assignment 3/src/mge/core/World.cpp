...
World::World():GameObject("root"), _mainCamera(0), _mainLight(0)
{
	_world = this;
}
...
Light* World::getMainLight() {
    return _mainLight; // or return _lights.empty() ? nullptr : _lights.front();
}


void World::setMainLight(Light* pLight) {
    if (pLight != NULL) _mainLight = pLight;
}